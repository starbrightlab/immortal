...
object I18n {
  ...
  private fun translateDict(text: String): String {
    return when (text) {
      ...
      "Install and set up Music Assistant as a server on your home network. New to Music Assistant? Learn more at music-assistant.io" -> "Instala y configura Music Assistant como servidor en tu red doméstica."
      "Install Snapcast from the Immortal App Store, and point it at your Music Assistant server." -> "Instala Snapcast desde la Tienda de Apps de Immortal y vincúlalo a tu servidor Music Assistant."
      "Turn on the toggle below, then enter your Music Assistant server's address." -> "Activa el interruptor inferior e introduce la dirección de tu servidor Music Assistant."
      ...
      "In Home Assistant, add the Mosquitto broker add-on (Settings → Add-ons) and the MQTT integration. New to MQTT? See home-assistant.io/integrations/mqtt" -> "En Home Assistant, añade el complemento Mosquitto broker (Ajustes → Complementos) y la integración MQTT."
      "Turn on the toggle below and enter your broker's address (and login, if any)." -> "Activa el interruptor inferior e introduce la dirección de tu servidor MQTT (y credenciales)."
      "This Portal appears automatically under Settings → Devices as a new MQTT device — no YAML needed." -> "Este Portal aparecerá automáticamente en Ajustes → Dispositivos como un nuevo dispositivo MQTT."
      ...
      "Sensors" -> "Sensores"
      "Ambient sensors" -> "Sensores ambientales"
      "Publish this Portal's ambient sensors (temperature, light) to Home Assistant. Only sensors this model actually has will appear." -> "Publica los sensores ambientales de este Portal (temperatura, luz) en Home Assistant. Solo aparecerán los sensores con los que cuente este modelo."
      "Temperature offset" -> "Ajuste de temperatura"
      "Correct the temperature reading. A sensor inside a powered device reads its own heat, so it usually runs a few degrees warm." -> "Corrige la lectura de temperatura. Un sensor dentro de un dispositivo encendido registra su propio calor, por lo que suele medir unos grados por encima."
      "Camera" -> "Cámara"
      "Let Home Assistant use this Portal's camera. Only switchable here - Home Assistant can't turn it on - and a badge stays on screen the whole time the camera is live." -> "Permite a Home Assistant usar la cámara de este Portal. Solo se puede activar aquí en el dispositivo (Home Assistant no puede encenderla) y un distintivo permanece en pantalla mientras esté activa."
      "Camera sound" -> "Sonido de la cámara"
      "Include sound in the video stream. Muting the microphone silences it, and an intercom announcement takes the microphone back." -> "Incluye sonido en la transmisión de vídeo. Silenciar el micrófono lo apaga, y un anuncio de intercomunicador toma el control del micrófono."
      "Stream video now" -> "Transmitir vídeo ahora"
      "Start the live RTSP stream from here as well as from Home Assistant. Play it at rtsp://127.0.0.1:8554/ on this Portal, or at the Portal's own address from elsewhere. Stops on reboot." -> "Inicia la transmisión RTSP en directo desde aquí además de desde Home Assistant. Reproduce en rtsp://127.0.0.1:8554/ en este Portal o en su dirección de red. Se detiene al reiniciar."
      "Publish to MQTT" -> "Publicar en MQTT"
      "Let Home Assistant use this Portal's camera as a video camera. Off by default, and only switchable here on the device - Home Assistant can't turn it on. The Portal shows a badge on screen the whole time the camera is live." -> "Permite a Home Assistant usar la cámara de este Portal como cámara de vídeo. Desactivado por defecto y solo configurable aquí en el dispositivo."
      "Include sound in the camera stream. Muting the microphone silences it, and a live intercom announcement takes the microphone back." -> "Incluye sonido en la transmisión de vídeo de la cámara. Silenciar el micrófono lo apaga."
